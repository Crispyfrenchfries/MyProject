		...
		<table class="table table-condensed table-striped">
			...
			<tr>
				...
				<c:set var="qstr" value="&findType=${findType}&findKeyword=${findKeyword}"/>
					...
					<li><a href="boardList.do?cpage=${prevBlock}${qstr}">이전 ${pagingBlock}개</a></li>
					</c:if>
					...
					<c:forEach var="i" begin="${prevBlock+1}" end="${nextBlock-1}">
						<c:if test="${i<=pageCount}">
							...
							><a href="boardList.do?cpage=${i}${qstr}">${i}</a></li>
						</c:if>
					</c:forEach>
					
					<c:if test="${nextBlock<=pageCount}">
					<li><a href="boardList.do?cpage=${nextBlock}${qstr}">이후 ${pagingBlock}개</a></li>
					</c:if>
					</ul>

...
<div class="row">
	...
	<br>
		<c:if test="${borad eq null }"> <!--  eq연산자는 ==와 동일 // 반대연산자 en(!=) -->
		<h3>존재하지 않는 글입니다.</h3>
		</c:if>
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<c:if test="${board.filename ne null}">
				<a href="<%=request.getContextPath()%>/FileDown?filename=${board.filename}">
				<!-- FileDownloadServlet과 매핑된 FileDown을 걸어주자 -->
				<c:out value="${board.filename}"/>
...
<script type="text/javascript">
	...
	var go=function(num, md){
			...
			//삭제시 한번 확인하기
			if(md==2){
				var yn=confirm(num+"번 글을 정말 삭제할까용?");
				if(!yn){
					return;
				}
			}
